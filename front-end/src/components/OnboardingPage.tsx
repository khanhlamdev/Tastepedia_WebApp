    ...
    <div className="min-h-screen bg-[#F2F4F7] flex items-center justify-center p-4 md:p-8 font-sans">
      <div className="w-full max-w-4xl bg-white rounded-3xl shadow-2xl p-8 md:p-14 flex flex-col min-h-[720px] relative mx-auto overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          {/* Skip button removed to force onboarding */}
        </div>
        ...
        <div className="flex gap-2.5 mb-12">
          {Array.from({ length: TOTAL_STEPS }).map((_, idx) => (
            <div
              key={idx}
              className={`h-1.5 flex-1 rounded-full transition-all duration-500 ease-out ${idx + 1 <= step ? 'bg-[#FF6B35]' : 'bg-gray-100'
                }`}
            />
          ))}
        </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-center">
                        ...
                        <h3 className={`font-bold text-2xl mb-2 transition-colors ${isActive ? 'text-[#FF6B35]' : s.color.text
                          }`}>
                          {s.label}
                        </h3>
